...
def enterRoom(request,ID,password):
	if ID in rooms.keys(): # If the room ID is found in the "rooms" dictionary...
		...
		if currRoom.occupied: # If the current room is occupied...
			...
			display = render_to_response('floor3/templates/html/floor3.html',roomList) # Update floor4 template with updated rooms
	...
	else: # If the room ID is NOT found in the "rooms" dictionary...
...
def exitRoom(request,ID,password):
	if ID in rooms.keys(): # If the room is found in the "rooms" dictionary...
		currRoom = rooms[ID] # Store the value for the room found
		if not currRoom.occupied: # If the current room is NOT occupied
			...
			roomList = createDic() # See creatDic() function above
			display = render_to_response('floor3/templates/html/floor3.html',roomList) # Update the floor4 template with updated rooms
			cache.set("display3",display,None) # Set new template as display3 variable
...
def createRooms(): # Fills the "rooms" dictionary
	roomIDs = cache.get("floor3") # Get all of the room IDs for rooms on floor 3
	for room in roomIDs: # For every room on floor 3...
		rooms[room] = Room(roomID = room, occupied = False) # Add an entry in the rooms dictionary, where each new room is initially NOT occupied
	roomList = createDic()
